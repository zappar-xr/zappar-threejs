...
setTimeout(() => {
  ...
  const geometry = new ZapparThree.TargetImagePreviewBufferGeometry(imageTracker.targets[0]);
  const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({color: 'red'}));
  scene.add(mesh);
  mesh.position.z = -5;
}, 1000);
function animate() {
